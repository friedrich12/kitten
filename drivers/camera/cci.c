...
void
CCI_INLINE_FUNC cci_int_get_status(struct cci_int_status *status)
{
        struct cci_int_ctrl_reg reg = {0};
        reg.val = __csi_read32(CCI_INT_CTRL);
...
void
CCI_INLINE_FUNC cci_int_clear_status(enum cci_int_sel interrupt)
{
        struct cci_int_ctrl_reg reg = {0};
        reg.val = __csi_read32(CCI_INT_CTRL);
        reg.s_tran_com_pd = 0;
...
enum cci_bus_status
CCI_INLINE_FUNC cci_get_bus_status()
{
        struct cci_ctrl_reg reg = {0};
        reg.val = __csi_read32(CCI_CTRL);
        return reg.cci_sta;
